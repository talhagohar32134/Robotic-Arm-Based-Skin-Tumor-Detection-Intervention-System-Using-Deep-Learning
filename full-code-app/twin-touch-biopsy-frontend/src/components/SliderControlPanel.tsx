...
export const SliderControlPanel = ({ 
  ...
  const handleElbowChange = (value: number[]) => {
    if (!emergencyStop) {
      onArmStateChange({ elbow: toRadians(value[0]) });
    }
  };
      ...
      <div className="p-4 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Precision 4-DOF Control</p>
          </div>
      ...
      <div className="p-4 space-y-4 flex-1 overflow-y-auto">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-mono font-bold text-primary">
              {toDegrees(armState.elbow)}°
            </span>
          </div>
          <Slider
            value={[armState.elbow * 180 / Math.PI]}
            min={-90}
            max={90}
            step={1}
            onValueChange={handleElbowChange}
            disabled={emergencyStop}
            className="w-full"
          />
          <div className="flex justify-between text-xs text-muted-foreground">
